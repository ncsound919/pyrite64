...
export type ColorMap = 'jet' | 'viridis' | 'hot' | 'cool' | 'grayscale';

/** Configuration for a single fluorescence channel (up to 4 channels). */
export interface FluorescenceChannel {
  /** Emission color (normalized RGB). */
  color:     [number, number, number];
  /** Emission intensity multiplier (0–4 typical). */
  intensity: number;
  /** Whether this channel is visible. */
  enabled:   boolean;
}

export interface BiotechImagingOptions {
  /** Initial visualization mode. Default: 'standard'. */
  mode?:         ImagingMode;
  /** Initial color map for false-color mode. Default: 'viridis'. */
  colorMap?:     ColorMap;
  /** Background darkness for fluorescence mode (0 = black, 1 = full). */
  bgDarkness?:   number;
  /** Confocal z-slice center (0–1, relative to scene depth). */
  zSlice?:       number;
  /** Confocal slice thickness (0–1). */
  zThickness?:   number;
  /** Phase-contrast halo strength (0–2). */
  phaseHalo?:    number;
}
...
const MODE_INDEX: Record<ImagingMode, number> = {
  'standard':       0,
  'fluorescence':   1,
  'confocal':       2,
  'false-color':    3,
...
const COLOR_MAP_INDEX: Record<ColorMap, number> = {
  'jet':       0,
  'viridis':   1,
  'hot':       2,
  'cool':      3,
...
export class BiotechImagingPass extends Pass {
  ...
  private _mode: ImagingMode;

  constructor(opts: BiotechImagingOptions = {}) {
    super();
   ...
   */
  setChannel(index: 0 | 1 | 2 | 3, ch: Partial<FluorescenceChannel>): void {
    const prefix = `ch${index}` as const;
  ...
  getChannel(index: 0 | 1 | 2 | 3): FluorescenceChannel {
    const prefix = `ch${index}` as const;
    const u = this.material.uniforms;
    const col = u[`${prefix}Color`].value as THREE.Vector3;
    const int_ = u[`${prefix}Intensity`].value as number;
    return {
  ...
  setZSlice(center: number, thickness: number): void {
  ...
  setLUTRange(contrast: number, brightness: number): void {
  ...
  setPhaseHalo(strength: number): void {
